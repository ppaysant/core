...
(function() {
	...
	FileList.prototype = {
		...
		/**
		 * File actions handler, defaults to OCA.UserLDAP.FileActions
		 */
		fileActions: null,
...
// 			if (!this.fileActions) {
// 				this.fileActions = new OCA.UserLDAP.FileActions();
...
// 			this.fileActions.display(filenameTd, !options.silent, this);
		...
		updateStorageStatistics: function(force) {
			OCA.UserLDAP.Files.updateStorageStatistics(this.getCurrentDirectory(), force);
		},

		getAjaxUrl: function(action, params) {
			return OCA.UserLDAP.Files.getAjaxUrl(action, params);
		},

		getDownloadUrl: function(files, dir) {
			return OCA.UserLDAP.Files.getDownloadUrl(files, dir || this.getCurrentDirectory());
		},
		...
		lazyLoadPreview : function(options) {
			...
			// get mime icon url
			OCA.UserLDAP.Files.getMimeIcon(mime, function(iconURL) {
				var previewURL,
					urlSpec = {};
				ready(iconURL); // set mimeicon URL

				urlSpec.file = OCA.UserLDAP.Files.fixPath(path);
				...
				if (etag){
					...
					console.warn('OCA.UserLDAP.FileList.lazyLoadPreview(): missing etag argument');
				}
		...
		rename: function(oldname) {
			...
			var checkInput = function () {
				...
				if (filename !== oldname) {
					...
					OCA.UserLDAP.Files.isFileNameValid(filename);
...
// 						self.$fileList.trigger($.Event('fileActionsReady', {fileList: self, $files: $(tr)}));
	...
	OCA.UserLDAP.DeletedUsersList = FileList;
})();
